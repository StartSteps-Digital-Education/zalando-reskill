package org.startsteps.week15.day4.interview;

public class Test {
    //1 Write a Java program and compute the sum of an integer's digits.
    //Input Data:
    //Input an integer: 25
    //Expected Output
    //
    //The sum of the digits is: 7





    //2  Write a Java program to compute the distance between two points on the earth's surface.
    //Distance between the two points [ (x1,y1) & (x2,y2)]
    //d = radius * arccos(sin(x1) * sin(x2) + cos(x1) * cos(x2) * cos(y1 - y2))
    //Radius of the earth r = 6371.01 Kilometers
    //Input Data:
    //Input the latitude of coordinate 1: 25
    //Input the longitude of coordinate 1: 35
    //Input the latitude of coordinate 2: 35.5
    //Input the longitude of coordinate 2: 25.5
    //Expected Output
    //
    //The distance between those points is: 1480.0848451069087 km



    //3 Write a Java program to find all numbers up to input n that are divisible by 7


    //4 Write a Java program to find the length of the longest consecutive sequence in a given array of integers.
    //
    //Original array: [1, 1, 2, 3, 3, 4, 5, 2, 4, 5, 6, 7, 8, 9, 6, -1, -2]
    //7 [1, 1, 2, 3, 3, 4, 5]

    //5 Write a Java program to add one to a positive number represented as an array of digits.
    //
    //Sample array: [9, 9, 9, 9] which represents 9999
    //Output: [1, 0, 0, 0, 0].
    //
    //Expected Output:
    //
    //Original array: [9, 9, 9, 9]
    //Array of digits: [1, 0, 0, 0, 0]
}
